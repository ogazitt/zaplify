...
namespace BuiltSteady.Zaplify.Tools.UserDataImport
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            bool success = DataImporter.Import(conn, file, user);
            if (success)
                Console.WriteLine("Succeeded in importing data");
            else
                Console.WriteLine("Failed to import user data");
        }

        private static void Usage()
        {
            Console.WriteLine(
                "Usage: UserDataImport.exe\n" +
